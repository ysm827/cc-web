...
**局域网访问**（手机和电脑在同一 WiFi）：
- 出于安全考虑，CC-Web 默认只监听 `127.0.0.1`，因此直接访问 `http://电脑局域网IP:8002` 默认不可用。
- 推荐通过 Nginx 等反向代理、Tailscale 或 Cloudflare Tunnel 暴露访问入口，并配合防火墙限制来源。
- 如果确认只在可信局域网内使用，可自行将服务监听地址从 `127.0.0.1` 改为 `0.0.0.0`（例如增加 `HOST=0.0.0.0` 配置并让启动代码读取该变量），再通过 `http://电脑局域网IP:8002` 访问。
- 不建议在公网服务器上直接暴露 `0.0.0.0:8002`。
